...
const page = ({params}) => {
  ...
  useEffect(()=>{
    if((session?.user._id && !sess) || (session?.user.id && !sess)){
      const fetchProjects = new Promise(async (res,rej)=>{
        await axios.get(`/api/project/get-result/${session?.user._id || session?.user.id}/${params.projectid}/${params.resultid}`)
        .then((response)=> {
  ...
  return (
    <>
      <ToastContainer />
      <RightPane pagename="My Projects" projectinfo={project} resultinfo={result} />
    </>
  )
}
